...
\setDNDlabel{OTHER PROFICIENCIES}{\scriptsize WEAPON, ARMOR, \&\ LANGUAGE PROFICIENCIES}
\setDNDlabel{MAIN PROFICIENCIES}{PROFICIENCIES}
...
\begin{proflist}
   \getDND*{MAIN PROFICIENCIES}
   \whenDNDnonempty{MAIN PROFICIENCIES}{\whenDNDnonempty{PROFICIENCIES}{\profskip}}
...
\node [anchor=south west,columnbox,minimum height=\sectionheight,
  ...
  {\nodepart{label}\getDNDlabel{OTHER PROFICIENCES}
   \nodepart{text}%
   \ifDNDnonempty{OTHER PROFICIENCIES}{%
       \multicolsep=0pt
      \begin{multicols}{2}
      \begin{proflist}
      \itemsep=1pt
      \getDND*{OTHER PROFICIENCIES}
      \end{proflist}
      \end{multicols}
   }{%
     (No other proficiencies specified.)
   }
  }
  ;
