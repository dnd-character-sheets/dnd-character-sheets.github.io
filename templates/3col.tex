...
\setDNDlabel{SECONDARY PROFICIENCIES}{\scriptsize WEAPON, ARMOR, \&\ LANGUAGE PROFICIENCIES}
\setDNDlabel{PROFICIENCIES}{PROFICIENCIES}
...
\begin{charsheet}
  ...
  \setDND{CLASS + LEVEL}{\getDND{CLASS} \getDND{LEVEL}} % use legacy layout below
...
%      \draw [thick,black,fill=yellow] (speed.labeltop) circle [radius=5pt];
     ...
     \whenDNDdefined{LEVEL}{
         \expandafter\ifnum\rawgetDND{LEVEL}<5
...
\node[columnbox,anchor=north east] (motivation) at (splash.east |- hpbackground.north)
   {\nodepart{label}DESCRIPTION \&\ PERSONALITY
    ...
    \ifDNDnonempty{SUGGESTED MOTIVATIONS}{%
       ...
       \begin{itemize}[nosep,leftmargin=1.5em]\getDND{SUGGESTED MOTIVATIONS}
       \end{itemize}
       \medskip
    }{}
    \ifDNDnonempty{SUGGESTED NAMES}{%
       Suggested names:
       \begin{itemize}[nosep,leftmargin=1.5em]\getDND{SUGGESTED NAMES}
       \end{itemize}
       \medskip
    }{}
    \hrule height 0pt depth 0pt % retain skips
   }
;



\newif\ifspellListOpen
\spellListOpenfalse
...
\setdeltay\sectionheight{motivation.south}{bottom right}


\whenDNDdefined{FEATURES}{%
  %%% XXX drops below bottom right
  \ifDNDnonempty{MAGIC}{\def\myfill{magic}}{\def\myfill{features}}%
  \node (features) [columnbox,height=\sectionheight-\mynodedistance,fill=\myfill,below=of motivation]  {
    \nodepart{label}{FEATURES \&\ TRAITS}\nodepart{text}%
    \raggedright
    \useDNDfont*{FEATURES}%
    \long\def\described#1#2{\strut\emph{#1:} #2\par\smallskip}%
    \long\def\describedColored#1#2#3{\described{#1}{#2}}%
    \getDND{FEATURES}
    \whenDNDnonempty{SENSES}{\described{Senses}{\getDND{SENSES}}}
    \whenDNDnonempty{MAGIC}{%
      \smallskip
      \footnotesize
      \useDNDfont*{MAGIC}%
      \newcommand\spellslevellabel[2]{%
        \ifnum#1=0
           \emph{\normalsize Cantrips:} You can cast these spells as often as you like using 1~action.
        \else
           \ifspellListOpen
             \end{itemize spells}
           \fi
           \smallskip
           \emph{\normalsize Level~#1 spells:} Cast these spells in any combination%
           \ifnum#2>0\relax
      ...
      \long\def\described#1#2{%
        \ifspellListOpen\else\begin{itemize spells}\fi
        \item #1: #2}%
      \long\def\describedColored#1#2#3{%
            \ifspellListOpen\else\begin{itemize spells}\fi
            \item \hspace*{-\fboxsep}{\colorbox{#1}{#2:} #3}%
      }
      \getDND{MAGIC}
      \ifspellListOpen
        \end{itemize spells}
...
\begin{proflist}
  \ifDNDnonempty{PROFICIENCIES}
     {\getDND{PROFICIENCIES}}
     {\item (No proficiencies listed.)}
\end{proflist}
   }
   ;
...
%   anchor=north,at=(hpbackground.south),
]
   {
    ...
    \DNDranges{rangeparagraphs}\rangeparagraph
...
\node (perception box)
      ...
      {\ifDNDnonempty{PASSIVE PERCEPTION}
         {\large\textsf{\rawgetDND{PASSIVE PERCEPTION}}}
         {?}
      }
      ;
...
\node (secondary proficiencies top) [below=of passive perception] { };

\setdeltay\sectionheight{secondary proficiencies top}{bottom left}
...
\node [anchor=south west,columnbox,minimum height=\sectionheight,
  ...
  {\nodepart{label}\getDNDlabel{SECONDARY PROFICIENCES}
   \nodepart{text}%
   \ifDNDnonempty{SECONDARY PROFICIENCIES}{%
       \multicolsep=0pt
      \begin{multicols}{2}
      \begin{proflist}
      \itemsep=1pt
     ...
     (No secondary proficiencies specified.)
   }
  }
  ;
