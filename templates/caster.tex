...
\ifDNDfalse{COLOR}{\newcommand\splashfile{splash-nocolor.png}\dndnocolor}
                  {\newcommand\splashfile{splash.png}}
...
\ifDNDdefined{FEATURES}{
  \ifDNDdefined{MAGIC}
   ...
   \whenDNDdefined{FEATURES SPACE}{\featurespostspace=\rawgetDND{FEATURES SPACE}}
...
}{\path (magic.south east) coordinate (features);} % anchor is to south east
...
\ifDNDdefined{EQUIPMENT}{%
   ...
   \ifDNDdefined{MAGIC}
        ...
        {\ifDNDnonempty{EQUIPMENT LEFT}
            {\def\where{left of lower corner=of features}
             \setdeltax\sectionwidth{features.west}{stats background.west}
             \advance\sectionwidth by -\mynodedistance
            }
