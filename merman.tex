...
\draw [opacity=0.7,fill=white] (merman.south west) rectangle (merman.north east);
...
\path (41,169.5) node (STR) {\getDND*{STR}}
...
\path (45,0) coordinate (left center);
...
\path (144.5,0 |- ac baseline) node {\getDND*{ARMOR CLASS}};

\path (161.5,0 |- ac baseline) node {\whenDNDdefined{INITIATIVE}{\signed{\rawgetDND{INITIATIVE}}}};

\path (190,0 |- hp baseline) node {\getDND*{SPEED}};

\tikzset{anchor=north}

\path (0, 230) coordinate (bg top);

\node (background) at (bg top -| left center) {\getDND*{BACKGROUND}};

\node (motivation) at (bg top -| center) {\getDND*{MOTIVATION}};
